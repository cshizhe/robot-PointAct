...
def main(args):
    ...
    with jsonlines.open(args.result_file, 'r') as f:
        for item in f:
            ckpt_step = int(os.path.basename(item['checkpoint']).split('.')[0].split('-')[-1]) if isinstance(item['checkpoint'], str) else item['checkpoint']
            if args.ckpt_step is not None and ckpt_step != args.ckpt_step:
                continue
            if (item['checkpoint'], item['task'], item['variation']) in taskvar_set:
                continue
            results[item['checkpoint']].append((item['task'], item['variation'], item['sr'], item['num_episodes']))
    ...
    if isinstance(ckpts[0], int):
        ...
        ckpts.sort(key=lambda x: int(os.path.basename(x).split('.')[0].split('-')[-1]))
